...
export async function apiSearchQuestionsEnhanced(req: EnhancedSearchRequest): Promise<EnhancedSearchResponse> {
  ...
  try {
    ...
    if (!(err instanceof StrataxApiError) || (err.status !== 404 && err.status !== 405)) {
      throw err;
    }

    console.warn(
      "[Intelligence API] /search/ultra-production not available (status",
      err.status,
      ") – falling back to /search/enhanced"
    );
    ...
    res = await strataxFetch(fallbackUrl.toString(), {
      method: "POST",
      headers: buildHeaders(),
...
export async function apiGetSourceStats(): Promise<SourceStatsResponse> {
  const res = await strataxFetch(`${INTELLIGENCE_BASE_URL}/sources/stats`, {
    method: "GET",
    headers: buildHeaders(),
  });
  return res.json();
...
export async function apiGetCompanies(): Promise<CompanyInfo[]> {
  const res = await strataxFetch(`${INTELLIGENCE_BASE_URL}/companies`, {
    method: "GET",
    headers: buildHeaders(),
  });
  return res.json();
...
export async function apiSubmitCommunityQuestion(params: { submitted_by: string; body: CommunitySubmitRequest }): Promise<CommunitySubmitResponse> {
  ...
  const res = await strataxFetch(url.toString(), {
    method: "POST",
    headers: buildHeaders(),
    body: JSON.stringify(body),
  });
...
export async function apiGetTransparency(): Promise<string> {
  const res = await strataxFetch(`${INTELLIGENCE_BASE_URL}/transparency`, {
    method: "GET",
    headers: buildHeaders(),
  });
  return res.text();
...
export async function apiGetEnhancedHealth(): Promise<any> {
  const res = await strataxFetch(`${INTELLIGENCE_BASE_URL}/health/enhanced`, {
    method: "GET",
    headers: buildHeaders(),
  });
  return res.json();
...
export async function apiGetTopics(): Promise<TopicsResponse> {
  const res = await strataxFetch(`${INTELLIGENCE_BASE_URL}/topics`, {
    method: "GET",
    headers: buildHeaders(),
  });
  const data = await res.json();
...
export async function apiTriggerUpdate(): Promise<UpdateResponse> {
  const res = await strataxFetch(`${INTELLIGENCE_BASE_URL}/update`, {
    method: "POST",
    headers: buildHeaders(),
  });
  const data = await res.json();
...
export async function apiGetHistoryTab(tabId: string): Promise<HistoryTabSummary> {
  ...
  const res = await strataxFetch(`${HISTORY_BASE_URL}${encodeURIComponent(tabId)}`, {
    method: "GET",
    headers: buildHeaders(),
  });
  return res.json();
...
export async function apiSaveHistoryTab(payload: {
  ...
  const res = await strataxFetch(HISTORY_BASE_URL, {
    method: "POST",
...
export async function apiGetHistoryStats(): Promise<HistoryStatsResponse> {
  // HISTORY_BASE_URL already ends with /, so no need to add another slash
  const res = await strataxFetch(`${HISTORY_BASE_URL}stats/overview`, {
    method: "GET",
  });
  return res.json();
}
...
export async function apiExportHistory(format: "json" | "csv" = "json"): Promise<Blob> {
  // HISTORY_BASE_URL already ends with /, so no need to add another slash
  const res = await strataxFetch(`${HISTORY_BASE_URL}export/${format}`, {
    method: "GET",
  });
  return res.blob();
}
